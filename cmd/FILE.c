...
#define O_DIRECTORYX	BIT(12)	/* Used internally - not a keyword option */
...
int main(int argc, char **argv)
{
	...
	if (protocol > 1) {
		logx(LOG_ERR, "FILE: protocol I can't handle: %d\n",
			protocol);
...
//	ioctl(0, CHIOCACCEPT, NOSTR);
	...
	logx(LOG_INFO, "FILE: exiting normally\n");
	finish(0);
}
/*
...
getwork(void)
{
	...
	for (;;) {
...
#endif
		if (length <= 0) {
			logx(LOG_INFO, "FILE: Ctl connection broken(%d,%d)\n",
				length, errno);
			tfree(t);
			return (TNULL);
		}
		...
		case CLSOP:
		case LOSOP:
			logx(LOG_INFO, "FILE: Close pkt: '%s'\n", p.cp_data);
			tfree(t);
			return TNULL;
		case RFCOP:
			continue;
 ...
 */
int tinit(struct transaction *t)
{
	t->t_tid = NOSTR;
	t->t_fh = FNULL;
	t->t_command = CNULL;
 ...
 */
int tfree(struct transaction *t)
{
	if (t->t_tid)
		free(t->t_tid);
	if (t->t_args)
...
{
	va_list ap;
	logx(LOG_ERR, "Fatal error in chaos file server: ");
	va_start(ap, s);
	logx(LOG_ERR, s, ap);
	va_end(ap);
	logx(LOG_ERR, "\n");
 ...
 */
int respond(struct transaction *t,char *results)
{
	int len;
	struct chpacket p;
 ...
 */
int error(struct transaction *t,char *fh,int code)
{
	struct chpacket p;
...
int syncmark(struct file_handle *f)
{
	...
	if (write(f->f_fd, &op, 1) != 1)
		return -1;
	if (log_verbose) {
		logx(LOG_INFO, "FILE: wrote syncmark to net\n");
	}
	return 0;
}

 ...
 */
int dataconn(struct transaction *t)
{
 ...
 */
int undataconn(struct transaction *t)
{
 ...
 * passwd.
 */
int login(struct transaction *t)
{
	struct passwd *p;
	struct cmdargs *a;
	int ufd;
...
logx(LOG_INFO, "FILE: %s %s failed", name, a->a_strings[1]);
logx(LOG_INFO, "p->pw_passwd %p, *p->pw_passwd %02x\n",
...
logx(LOG_INFO, "FILE: login() pw ok\n");
...
logx(LOG_INFO, "FILE: login() responding\n");
		respond(t, response);
		logx(LOG_NOTICE, "FILE: logged in as %s from host %s\n",
 ...
 */
int fileopen(struct transaction *t)
{
...
int getprops(struct transaction *t)
{
 ...
 */
int propopen(struct xfer *x,struct transaction *t)
{
	char *cp;
 ...
 */
int directory(struct transaction *t)
{
	struct cmdargs *a = t->t_args;
	struct xfer *x;
	int errcode = 0;
	...
	if (a->a_options & ~(O_DELETED|O_FAST|O_SORTED)) {
		...
		if (!(a->a_options & (O_BINARY | O_CHARACTER)))
		 	a->a_options |= O_CHARACTER;
		x = makexfer(t, a->a_options | O_DIRECTORYX);
		x->x_realname = realname;
		x->x_dirname = dirname;
		x->x_state = X_IDLE;
		if ((errcode = startxfer(x)) == 0) {
 ...
 */
int diropen(struct xfer *ax,struct transaction *t)
{
	struct xfer *x = ax;
 ...
 */
int xcommand(struct transaction *t)
{
	struct file_handle *f;
	struct xfer *x;

	logx(LOG_INFO, "FILE: transfer command: %s\n", t->t_command->c_name);
 ...
 */
int xqueue(struct transaction *,struct xfer *x)
{
 ...
 * appropriate.
 */
int fileclose(struct xfer *x,struct transaction *t)
{
	x->x_flags |= X_CLOSE;
	switch (x->x_options & (O_READ | O_WRITE | O_DIRECTORYX | O_PROPERTIES)) {
	case O_READ:
	case O_DIRECTORYX:
	case O_PROPERTIES:
		/*
...
int xclose(struct xfer *ax)
{
	...
	(void)write(ctlpipe[1], (char *)&ax, sizeof(x)); 
#endif 
        if (x->x_options & (O_DIRECTORYX|O_PROPERTIES)) {
		respond(t, NOSTR);
		return 0;
	}
	/*
	...
	if (errcode == 0) {
		if (fstat(x->x_fd, &sbuf) < 0) {
			logx(LOG_ERR, "fd:%d, pid:%d, mypid:%d, errno:%d\n", x->x_fd,
					getpid(), mypid, errno);
 ...
 */
int filepos(struct xfer *x,struct transaction *t)
{
 ...
 */
int filecontinue(struct xfer *x,struct transaction *t)
{
 ...
 * realname.
 */
int delete(struct transaction *t)
{
	struct file_handle *f = t->t_fh;
	struct xfer *x;
	...
	if (f != FNULL)
		if (t->t_args != ANULL && t->t_args->a_strings[0] != NOSTR) {
			...
			error(t, f->f_name, BUG);
		} else if (f->f_xfer->x_options & (O_PROPERTIES|O_DIRECTORYX)) {
			errstring =
 ...
 * Rename a file.
 */
int xrename(struct transaction *t)
{
	struct file_handle *f;
	struct xfer *x;
	int errcode;
	...
	if ((errcode = parsepath(file1, &dir1, &real1, 0)) != 0)
		...
		if (file2 != NOSTR) {
			...
			error(t, f->f_name, BUG);
		} else if (x->x_options & (O_DIRECTORYX|O_PROPERTIES)) {
...
int complete(struct transaction *t)
{
	char *cp, *tp;
	...
	if (dfile[0] != '/') {
		...
		if (log_verbose) {
			logx(LOG_INFO, "ifile:'%s'\nireal:'%s'\nidir:'%s'\n",
			    ...
			    ireal ? ireal : "!",
			    idir ? idir : "!");
			logx(LOG_INFO, "dfile:'%s'\ndreal:'%s'\nddir:'%s'\n",
			    dfile ? dfile : "!",
			    dreal ? dreal : "!",
			    ddir ? ddir : "!");
			logx(LOG_INFO, "iname:'%s'\nitype:'%s'\n",
			    iname ? iname : "!",
			    itype ? itype : "!");
			logx(LOG_INFO, "dname:'%s'\ndtype:'%s'\n",
			    dname ? dname : "!",
			    dtype ? dtype : "!");
			logx(LOG_INFO, "adir:'%s'\n",
			    adir ? adir : "!");
		}
#if !defined(BSD42) && !defined(linux) && !defined(OSX)
		...
		while( (dirp = readdir(dfd)) != NULL ) {
			...
			if (log_verbose) {
				logx(LOG_INFO, "top of readdir loop; '%s'\n",
				    dirp->d_name);
			}
			...
			if ((namematch = prefix(iname, ename)) == SNONE ||
				...
				continue;
			if (log_verbose) {
				logx(LOG_INFO, "ename:'%s' etype:'%s'\n",
				    ename ? ename : "!",
				    etype ? etype : "!");
				logx(LOG_INFO, "nm:%d, tm:%d, ns:%d, ts:%d\n",
			...
			if (log_verbose) {
				logx(LOG_INFO, "aname:'%s', atype:'%s'\n",
				    aname ? aname : "!",
				    atype ? atype : "!");
				logx(LOG_INFO, "nstate: %d, tstate: %d\n",
				    nstate, tstate);
			}
		}
gotit:
...
int incommon(char *old, char *new)
{
 ...
 * Create a directory
 */
int crdir(struct transaction *t)
{
 ...
 */
int expunge(struct transaction *t)
{
 ...
 */
int chngprop(struct transaction *t)
{
	struct file_handle *f = t->t_fh;
	struct xfer *x = XNULL;
	int errcode;
 ...
 */
int fherror(struct file_handle *f,int code,int type,char *message)
{
	struct file_error *e = &errors[code];
	struct chpacket pkt;
	int len;
	...
	case O_DIRECTORYX:
		...
		while (x->x_room != 0) {
			if (x->x_left == 0) {
				...
				n = x->x_options & O_DIRECTORYX ? dirread(x) :
...
int xbwrite(struct xfer *x)
{
	...
	if ((ret = write(x->x_fd, x->x_bbuf, n)) <= 0) {
		logx(LOG_ERR, "FILE: write error %d (%d) to file\n",
			ret, errno);
		return -1;
	}
	if (log_verbose) {
		logx(LOG_INFO,"FILE: wrote %d bytes to file\n", ret);
		logx(LOG_INFO,"FILE: fd %d\n", x->x_fd);
	}
	return 0;
}
...
xpweof(struct xfer *x)
{
	...
	{
        	logx(LOG_ERR, "FILE: xpweof write failed (errno %d)\n", errno);
                int tries = 0;
                do {
                        usleep(100);
                        if (write(x->x_fh->f_fd, &op, 1) == 1) {
                                logx(LOG_ERR, "FILE: xpweof retry succeeded\n");
                                return 0;
                        }
                } while (tries++ < 200000);
                logx(LOG_ERR, "FILE: exhausted retries, xpweof (errno %d)\n", errno);
                return -1;
	}
        if (log_verbose) {
                logx(LOG_INFO, "FILE: wrote EOF to net\n");
        }
        return 0;
}
...
xpwrite(struct xfer *x)
{
        ...
        if ((ret = write(x->x_fh->f_fd, (char *)&x->x_pkt, len)) != len) {
                ...
                do {
                        usleep(100);
                        if ((ret = write(x->x_fh->f_fd, (char *)&x->x_pkt, len)) == len) {
                                logx(LOG_ERR, "FILE: write error retry succeeded\n");
                                return 0;
                        }
                } while (tries++ < 200000);
                logx(LOG_ERR, "FILE: exhausted retries, write error %d (errno %d) to file %d\n",ret, errno);
                return -1;
        }
        return 0;
}
...
loop:	
	...
	if (log_verbose) {
		logx(LOG_INFO, "FILE: read (%d) %d bytes from net\n",
		    x->x_op & 0377, n);
	}
	if (n < 0)
		return -1;
	...
	case DWDOP:
		if (n == 1) {
			logx(LOG_ERR, "FILE: zero size data packet\n");
			goto loop;	/* Zero size data packet!? */
		}
		return n - 1;
	default:
		logx(LOG_ERR, "FILE: bad opcode in data connection: %d\n",
			x->x_op & 0377);
...
put on runq or something, set up which fd and why to wait....
	...
	case -1:
		...
		return NER;
	default:
		if (!(x->x_options & (O_PROPERTIES|O_DIRECTORYX)))
			(void)close(x->x_fd);
		x->x_fd = -1;
		(void)close(pfd[0]);
		return 0;
	...
	case 0:
		...
		for (;;) {
			...
			if (log_verbose) {
				logx(LOG_INFO, "Switch pos: %ld, status: %ld\n",
			...
			case X_HANG:		/* Need more instructions */
				(void)signal(SIGHUP, SIG_IGN);
				rcvcmd(x);
				logx(LOG_INFO, "Hang pos: %ld\n", tell(x->x_fd));
				continue;
			}
			logx(LOG_INFO, "FILE: subproc exiting\n");
			exit(0);
		}
	}
	/* NOTREACHED */
...
#ifndef SELECT
int mdatfh(struct xfer *x,struct transaction *t)
{
...
int adatfh(struct xfer *x,struct transaction *t)
{
...
int delfh(struct xfer *x,struct transaction *t)
{
	x->x_flags |= X_DELETE;
	tfree(t);
}

int renfh(struct xfer *x,struct transaction *t)
{
	free(x->x_realname);
 ...
 */
int sendcmd(struct transaction *t,struct xfer *x)
{
	struct cmdargs *a;
	struct pmesg pm;
	...
	if (write(x->x_pfd, (char *)&pm, sizeof(pm)) != sizeof(pm) ||
		...
		logx(LOG_INFO, "FILE: send %s command\n", t->t_command->c_name);
		tfree(t);
	}
}
...
int rcvcmd(struct xfer *x)
{
	...
	if (pm.pm_args) {
		...
		t->t_args = ANULL;
	logx(LOG_INFO, "FILE: rcvd %s command\n", t->t_command->c_name);
