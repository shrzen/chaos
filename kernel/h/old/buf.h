...
/*
 * RCS Info	
 *	$Header: /home/ams/c-rcs/chaos-2000-07-03/kernel/h/old/buf.h,v 1.1.1.1 1998/09/07 18:56:08 brad Exp $
 *	$Locker:  $
 */
...
#ifdef RVD
	unsigned long	b_bitmap;	/* bitmap for remote disks */
	struct r_chain	*m_chain;	/* cache of ptes for physio for remote disks */
#endif RVD
};

#ifdef RVD

#define VDMAXBURST	32		/* Should be defined in vdreg.h */

struct r_chain {
	struct	r_chain *next;
	u_long	offset;
/* This is a large crock, but then again this is UNIX so no one will notice.
   The reason we do this is so that people can include buf.h without needing
   to include pte.h... why is this necessary??? */
/* NVD is only defined when the guy who included you, also included "vd.h" */
#ifdef NVD
	struct pte pmap[VDMAXBURST+2];
#else
	u_long	pmap[VDMAXBURST+2];
#endif NVD
	};

/* NCHAINS should be roughly the size of nbuf (the number of bufs created) */

#define NCHAINS	32	/* God help us if we really need more than this... */
