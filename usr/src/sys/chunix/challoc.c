...
static int bufwait;		/* Someone's waiting for buffers */
static struct buf *buflist;	/* Unassigned buffers */
int	Chnbufs;
...
again:
	for (sp = chsizes; sp < &chsizes[NSIZES]; sp++) {
		...
		if (sp->ch_buffree == 0) {
			...
			sp->ch_bufptr = bp;
			bp->b_nfree = j = BSIZE / sp->ch_bufsize;
			bp->b_bits = 0;
