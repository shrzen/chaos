...
static int		unmatched;	/* Unmatched RFC device is open */
...
dev_t dev;
{
	...
	if (conn == NOCONN) {
		...
		CHFCONN(fp) = conn;
		fp->f_flag |= FMP;		/* No seeking on this file */
		if (wstate) {
			/*
...
dev_t dev;
{
	...
	conn = CHFCONN(getf(u.u_arg[0]));
	chread(conn);
}
/*
 * Read from an open channel. Channels used as ttys are done specially.
 */
chread(conn)
register struct connection *conn;
{
	...
	case CHRECORD:
		...
		if ((pkt = conn->cn_rhead) == NOPKT ||
			...
			iomove(&pkt->pk_op, 1, B_READ);
			iomove(pkt->pk_cdata, pkt->pk_len, B_READ);
			spl6();
			ch_read(conn);
			spl0();
		}
...
/*
 * Common write routine.  Note that user programs can write to a connection
...
caddr_t addr;
{
	...
	default:
		if (conn->cn_state != CSOPEN || conn->cn_mode != CHTTY)
			break;
