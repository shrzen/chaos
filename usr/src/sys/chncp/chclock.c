...
ch_clock()
{
	...
	static int nextclk = 1;		/* next time to do anything */
	static int nextprobe = 1;	/* next time to probe */
	static int nexthang = 1;	/* next time to chxtime() */
	static int nextroute = 1;	/* next time to age routing */
	static int nextbridge = 1;	/* next time to send routing */

	if (nextclk != ++Chclock)
		return;
	nextclk += MINRATE;
	if ((int)(nextprobe - Chclock) <= 0) {
		probing = 1;
		nextprobe += PROBERATE;
	} else
		probing = 0;
	if ((int)(nexthang - Chclock) <= 0) {
		chxtime();
		nexthang += HANGRATE;
	}
	if ((int)(nextroute - Chclock) <= 0) {
		chroutage();
		nextroute += ROUTERATE;
	}
	if ((int)(nextbridge - Chclock) <= 0) {
		chbridge();
	...
	for (connptr = &Chconntab[0]; connptr < &Chconntab[CHNCONNS]; connptr++)
		if ((conn = *connptr) == NOCONN)
			...
			if ((pkt = conn->cn_toutput) != NOPKT &&
			    (unsigned)(Chclock - pkt->pk_time) >= ORATE &&
			    !chtfull(conn)) {
			...
			} else if (pkt != NOPKT && (unsigned)(Chclock - pkt->pk_time) >= RFCRATE) {
...
struct connection *conn;
{
	...
	for (opkt = &conn->cn_thead; pkt = *opkt;)
		if ((unsigned)(Chclock - pkt->pk_time) >= AGERATE) {
			if (firstpkt == NOPKT) 
				firstpkt = pkt;
			else
				lastpkt->pk_next = pkt;
...
chroutage()
{
	...
	for (r = Chroutetab; r < &Chroutetab[CHNSUBNET]; r++)
		if (r->rt_type == CHBRIDGE && r->rt_cost < CHHCOST)
			r->rt_cost++;
}
/*
...
chbridge()
{
	...
	register int naccess;	/* number of accessible subnets */
	register int n;
	...
	if (n <= 1 ||
	    (pkt = pkalloc(naccess * sizeof(struct rut_data), 1)) == NOPKT)
		return;
	/*
	 ...
	 */
	pkt->pk_len = naccess * sizeof(struct rut_data);
	pkt->pk_op = RUTOP;
	 ...
	 * naccess becomes zero on last such subnet.
	 */
	for (r = Chroutetab; r < &Chroutetab[CHNSUBNET]; r++)
		if (r->rt_type == CHDIRECT)
			sendrut(pkt, r->rt_xcvr, r->rt_cost, --naccess);
}
