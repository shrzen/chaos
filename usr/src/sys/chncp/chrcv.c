...
#define send_los(x,y) sendlos(x,y,sizeof(y))
...
ignore:
	 ...
	 */
		...
		send_los(pkt, "Bad index in packet");
	/*
...
register struct packet *pkt;
{
	 ...
	 */
	for (npkt = pkt;
	     npkt != NOPKT && npkt->pk_pkn == (short)(conn->cn_rlast + 1);
	     npkt = npkt->pk_next) {
...
struct packet *pkt;
{
	...
	rd = pkt->pk_rutdata;
	if (pkt->pk_ssubnet < 0 || pkt->pk_ssubnet >= CHNSUBNET)
		...
		if (rd->pk_subnet < 0 || rd->pk_subnet >= CHNSUBNET) {
