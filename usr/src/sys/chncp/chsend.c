 ...
 * started via XMITSTART if no packet is currently being transmitted.
 ...
 * XMITSTART is used by both to restart the transmitter if it is idle.
 * Note also that XMITTAIL, XMITHEAD and XMITSTART are not simple statements
 * thus should not be used in conditonals without braces.
 */
#define XMITHEAD(pkt, xcvr) \
