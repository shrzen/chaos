...
char *argv[];
{
	...
	if (net != 2) {
		...
		lose1:		close(tord[0]);
			...
			sprintf(contact, "%s/%d/%s",
				CHRFCADEV, c, cname?cname:"SUPDUP");
	} else {
	arpa:
		c = chaos_addr(via);
		if (c == 0) {
		...
		sprintf(contact, "%s/%d/ARPA %s %s",
			CHRFCADEV, c, host->host_name, cname?cname:"137");
		fixhostname(contact);
	}
	printf("Trying...");
	fflush(stdout);
	if ((conn = open(contact, 2)) < 0) {
...
char *argv[];
{
	...
	if (conn >= 0) {
		...
		ioctl(conn, CHIOCREJECT, "");	/* must send a CLS explicitly */
		close(conn);
		if (fin != NULL) {
			fclose(fin);
			fclose(fout);
