...
char	term[64] = "TERM=";
char	user[20] = "USER=";
char	*envinit[] = {homedir, shell, "PATH=:/usr/ucb:/bin:/usr/bin", term, user, 0, 0};
/* NOTE WELL extra zero at the end of envinit - it is for possible TERMCAP */
char	*getenv();
struct	passwd *pwd;
	...
	do {
		...
		if (*pwd->pw_passwd != '\0') {
			...
			if (strcmp(namep, pwd->pw_passwd))
				invalid = TRUE;
		} else if (ttyn[sizeof("/dev/tty")-1] == 'p' ||
			   ttyn[sizeof("/dev/tty")-1] == 'd') {
			getpass("Password:");
			invalid = TRUE;
		}
...
char	*ttyid;
{
	...
	if (ttyid == NULL)
		return (UNKNOWN);
	/*
	 * Kludge to make supdup win:
	 * First, if we are logging from the shell, we preserve the existing
	 * TERM and TERMCAP.
	 */
#define SDTERMCAP	"/tmp/sd-"
#define TERMCAP		"TERMCAP="
	if ((t = getenv("TERM")) != NULL && strcmp("supdup", t) == 0) {
		...
		return (t);
	/*
	 * If tty is a pty (is ttyp?), make the type "supdup" and read the
	 * termcap from where the supdup server put it and put in the
	 * environment in TERMCAP.
	 * The file is deleted after it is read.
	 * This stuff assumed ttypX naming of pty's and SDTERMCAP file names
	 */
	} else if ((p = rindex(ttyid, '/') + 1)[strlen("tty")] == 'p') {
		char file[sizeof(SDTERMCAP) + sizeof("ttypX") + 2];
		struct stat sbuf;
		char **ep, *tp, *malloc(), *strcpy(), *sprintf();
		int fd;

		(void)sprintf(file, "%s%s", SDTERMCAP, p);
		if ((fd = open(file, 0)) >= 0 &&
		    unlink(file) >= 0 &&
		    fstat(fd, &sbuf) >= 0 &&
		    (tp = malloc((unsigned)sbuf.st_size + sizeof(TERMCAP) + 1))) {
			(void)strcpy(tp, TERMCAP);
			(void)read(fd, tp + strlen(tp), (int)sbuf.st_size);
			tp[strlen(TERMCAP) + (int)sbuf.st_size] = '\0';
			for (ep = envinit; *ep; ep++)
				;
			*ep = tp;
			close(fd);
