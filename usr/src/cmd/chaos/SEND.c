...
char *argv[];
{
	...
	if(argc < 2) {
		ioctl(0, CHIOCREJECT, "No user name supplied");
		exit(1);
	}
	him = argv[1];
	lcase(him);
...
cont:
	if (logcnt==0 && histty[0]=='\0') {
		ioctl(0, CHIOCREJECT, "User not logged in.");
		exit(1);
	}
	fclose(uf);
	if(histty[0] == 0) {
		ioctl(0, CHIOCREJECT, "User not logged in out given tty.");
		exit(1);
	}
	if (access(histty, 0) < 0) {
		ioctl(0, CHIOCREJECT, "User's tty non-existent.");
		exit(1);
	}
	...
	*sptr++ = '\f';
	sprintf(sptr, "TTY message from chaosnet site %s\n",
		chaos_name(chst.st_fhost));
	sptr += strlen(sptr);
	while ((c1 = getchar()) != EOF)
		if (sptr >= &sendbuf[MAXSEND])
			break;
		else if ((char)c1 == CHNL)
			*sptr++ = '\n';
		else
			*sptr++ = c1;
	...
	if (him[0] != '-' &&
		...
		if ((pwd = getpwnam(him)) == NULL ||
		    chown(of, pwd->pw_uid) < 0)
			chmod(of, 0666);
		lseek(ofd, 0L, 2);
		write(ofd, sendbuf, sptr - sendbuf);
	}
	printf("\7\n\r----------------------------------------\n\r");
	*sptr = '\0';
	...
	exit(0);
perm:
	ioctl(0, CHIOCREJECT, "Permission denied on his tty.");
	exit(1);
}
...
timout()
{
	...
	ioctl(0, CHIOCREJECT, "Timeout opening their tty.");
	exit(1);
}
