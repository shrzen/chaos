...
char *name;
{
	...
	*host++ = 0;	/* split into host, name */
	...
	if (address != 0) {
		if (large)
			sprintf(contact, "/dev/chaos/%d/NAME /W %s",
				address, name);
		else
			sprintf(contact, "/dev/chaos/%d/NAME %s",
				address, name);
	} else {
		sprintf(contact, "/dev/chaos/%d/ARPA %s 117",
			chaos_addr("mit-mc", 0), fname);
		fixhostname(contact);
		mode = 2;	/* have to write to the ARPA side */
	}
	if ((c = open(contact, mode)) < 0) {
