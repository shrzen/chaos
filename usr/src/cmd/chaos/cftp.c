...
char *rfcargs;
{
	...
	if ((c = (CONN)malloc(sizeof *c)) == (CONN)NULL)
		return((CONN)NULL);
	sprintf(contact, "%s/%d/%s", CHRFCADEV, chaos_addr(host), rfcargs);
	if ((c->file = open(contact, 2)) < 0) {
...
char *cname;
{
	...
	if ((c = (CONN)malloc(sizeof *c)) == (CONN)NULL)
		return((CONN)NULL);
	sprintf(contact, "%s/%s", CHLISTDEV, cname);
	if ((c->file = open(contact, 2)) < 0) {
		free((char *)c);
		return((CONN)NULL);
	}
...
register CONN c;
{
	...
	if (debug) {
		if (n > 0)
			...
			printf("%s ", c == control ? "Control" : "Data");
...
char *argv[];
{
	...
	if ((control = chopen(name, "FILE")) == NULL) {
...
opendata()
{
	...
	ctl_receive();
	chrpkt(data);	/* read the RFC packet */
	return(1);
}
...
register long bytes;
{
	...
	if (delta != 0)
		printf(" = %g Kbits per second", bytes*(8./1000.)/delta);
	putchar('\n');
}
...
#define NBPL 16	/* number of bits per long */
...
register FILE *f;
{
	...
	register int pos;
	register int mask;
	register int lim = NBPL - bytesize;
	bytes = 0;
	mask = ~(~0 << bytesize);
	chwop(0300, data);
	l = 0;
	while ((l = getc(f)) != EOF) {
		if ((w = getc(f)) != EOF)
			l |= w << 8;
		for (pos = 0; pos <= lim; pos += bytesize, l >>= bytesize) {
			w = l & mask;
			chwrite(w & 0377, data);
			chwrite(w >> 8, data);
			bytes += 2;
...
register FILE *t;
{
	...
	bytes = 0;
	pos = 0;
	while ((w = chread(data)) != EOF) {
		w += chread(data) << 8;
		bytes += 2;
		w &= mask;
		l |= w << pos;
		if ((pos += bytesize) > lim) {
			fwrite(&l, sizeof l, 1, t);
			pos = 0;
			l = 0;
		}
	}
